                ...
                .each(function () {
                    ...
                    THEMEREX_Swipers[id] = new Swiper('.' + id, {
                        ...
                        onFirstInit: function (slider) {
                            ...
                            if (!cont.hasClass('sc_slider_autoheight'))
                                return;
...
function prepareSliderNavi(slider) {
    ...
    if (navi.length == 0)
        navi = slider.siblings('.flex-direction-nav');
    if (navi.length > 0) {
        navi.find('.flex-prev').click(function (e) {
            var swiper = jQuery(this).parents('.swiper-slider-container');
            if (swiper.length == 0)
                swiper = jQuery(this).parents('.flex-direction-nav').siblings('.swiper-slider-container');
...
function calcSliderDimensions(slider) {
    if (slider.height() == 0 || slider.hasClass('sc_slider_noresize') || !slider.hasClass('inited'))
        return;
    ...
    if (isNaN(w) || w < 50 || isNaN(h) || h < 50) {
        ...
        if (w < slider.width())
            slider.width(w);	// Prevent to make slider dimensions bigger then original
